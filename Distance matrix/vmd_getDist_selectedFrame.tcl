...
#  if {$frame in $flist} {
# set idx [lsearch $flist $frame]
# set flist [lreplace $flist $idx $idx]
# exec echo $flist >> flist.dat
#  if {$frame == [lindex $flist 0]} {
...
proc fResPair { frame } {
  ...
  set fall [open "contact_all_temp2.dat" a+]
 ...
 # go through the pairs, assign distances
	...
	foreach name [array names contactTable] {
      ...
      puts $fall [format "%-5d  %f" $frame  $minDistance]
      flush $fall
